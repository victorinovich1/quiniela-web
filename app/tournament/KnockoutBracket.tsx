    ...
    <div className="space-y-6">
      ...
      <div className="relative overflow-x-auto pb-4">
        <div className="min-w-[1400px] max-w-full mx-auto py-6">
          <div className="grid grid-cols-7 gap-x-6 min-h-[850px] items-center justify-center">
            
            {/* COLUMNA 1: R32 Extremo Izquierdo */}
            <div className="flex flex-col justify-around h-full relative">
              <div className="text-[9px] uppercase tracking-wider text-fifaGreen/50 font-bold text-center mb-3">
                DIECISEISAVOS
              </div>
              <div className="flex flex-col justify-around flex-1 space-y-2">
                {col1Ids.map(id => {
                ...
                {col2Ids.map((id, idx) => {
                  ...
                  return (
                    <div key={id} className="relative">
                      {/* Conector de entrada desde R32 */}
                      <div className="absolute -left-6 top-1/2 w-6 h-[1px] bg-white/10"></div>
                      <MatchCard match={m} homeTeam={teamsById[m.home_team_id!]} awayTeam={teamsById[m.away_team_id!]} />
                    </div>
                  )
                })}
              </div>
            ...
            {/* COLUMNA 4: CENTRO (SF1 → Final → SF2 → 3er lugar) */}
            <div className="flex flex-col justify-center h-full px-3 relative">
              <div className="text-[10px] uppercase tracking-wider text-gold/70 font-black text-center mb-4">
                🏆 FINAL 2026
              </div>
            ...
            <div className="flex flex-col justify-around h-full relative">
              ...
              <div className="flex flex-col justify-around flex-1 space-y-8">
                {col5Ids.map((id, idx) => {
                  ...
                  return (
                    <div key={id} className="relative">
                      {/* Conector de salida hacia SF */}
                      <div className="absolute -right-6 top-1/2 w-6 h-[1px] bg-white/10"></div>
                      <MatchCard match={m} homeTeam={teamsById[m.home_team_id!]} awayTeam={teamsById[m.away_team_id!]} />
                    </div>
                  )
                })}
...
function MatchCard({ match, homeTeam, awayTeam }: { match: Match, homeTeam: Team | null, awayTeam: Team | null }) {
  ...
  return (
    <div className={`w-[150px] h-20 bg-navy-dark border rounded-lg overflow-hidden transition-all hover:border-fifaGreen/50 ${
      ...
      {/* Teams */}
      <div className="p-1.5 space-y-0.5 flex flex-col justify-center h-[calc(100%-28px)]">
...
function FinalMatch({ match, homeTeam, awayTeam }: { match: Match, homeTeam: Team | null, awayTeam: Team | null }) {
  ...
  return (
    <div className={`w-[180px] min-h-[96px] bg-gradient-to-br from-navy-dark via-navy-deepest to-navy-dark border-2 rounded-xl overflow-hidden shadow-2xl transition-all ${
      isLive ? 'border-red-500 shadow-red-500/40 animate-pulse' : 'border-gold shadow-gold/30'
    }`}>
      <div className="bg-gradient-to-r from-gold/30 via-gold/20 to-gold/30 px-3 py-1 flex items-center justify-between">
      ...
      {champion && isFinished && (
        <div className="bg-gold/20 border-t border-gold/40 px-3 py-1 text-center">
          <span className="text-[8px] uppercase tracking-widest text-gold font-black">
...
function TeamRow({ 
  ...
  return (
    <div className={`flex items-center justify-between rounded px-1.5 py-1 transition-all ${
      isWinner ? 'bg-fifaGreen/20 border border-fifaGreen/50' : 'bg-white/5 border border-transparent'
    }`}>
      <div className="flex items-center gap-1.5 min-w-0 flex-1">
        {team ? (
          <>
            <Flag team={team} size={10} className="flex-shrink-0" />
          ...
          <span className="text-[9px] text-white/40 italic truncate">{label || 'Ganador MXX'}</span>
        )}
      </div>
      {score !== null && (
        <span className={`text-xs font-black ml-1.5 tabular-nums ${
...
function FinalTeamRow({ 
  ...
  return (
    <div className={`flex items-center justify-between rounded-lg px-2 py-1.5 transition-all ${
      ...
      <div className="flex items-center gap-2 min-w-0 flex-1">
        {team ? (
          ...
          <span className="text-[10px] text-white/50 italic truncate">{label || 'TBD'}</span>
        )}
      </div>
      {score !== null && (
        <span className={`text-base font-black ml-2 tabular-nums ${
