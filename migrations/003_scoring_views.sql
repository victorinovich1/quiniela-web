-- =====================================================
-- 003 - Vistas de puntuación (versión inicial con user_id)
-- Reemplazada en migración 010 con entry_id.
-- =====================================================
...
create or replace view public.teams_in_phase as
with finals as (
...
select
  ...
  + case when sp.top_scorer is not null and lower(trim(sp.top_scorer)) = lower(trim(coalesce(s.top_scorer,''))) and s.top_scorer is not null then s.pt_top_scorer else 0 end
  + case when sp.mvp is not null and lower(trim(sp.mvp)) = lower(trim(coalesce(s.mvp,''))) and s.mvp is not null then s.pt_mvp else 0 end
  + case when sp.best_goalkeeper is not null and lower(trim(sp.best_goalkeeper)) = lower(trim(coalesce(s.best_goalkeeper,''))) and s.best_goalkeeper is not null then s.pt_goalkeeper else 0 end
...
select
  ...
  pr.alias,
  pr.paid,
  coalesce((select sum(points) from public.match_scores ms where ms.user_id = pr.id), 0) as match_points,
  coalesce((select points from public.special_scores ss where ss.user_id = pr.id), 0) as special_points,
  coalesce((select sum(points) from public.match_scores ms where ms.user_id = pr.id), 0)
  + coalesce((select points from public.special_scores ss where ss.user_id = pr.id), 0) as total_points
